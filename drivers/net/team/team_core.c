...
/* Upstream commit 425000dbf173 passes the lower device to ->parse, which
 * relies on the 3-arg header_ops.parse introduced later than this tree's
 * base. Here header_ops.parse is 2-arg, so recover the team device from
 * skb->dev and forward to the lower parse; the create path (the actual
 * type-confusion crash) is fixed identically to upstream.
 */
